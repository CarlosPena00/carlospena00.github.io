...
- O **`B7`** é o único acorde de 4 notas do conjunto. Os outros seis são de 3, então a mão inteira muda de configuração só nele.
- O **`A → D`** é o único salto da sequência. O `A` aparece nos dois acordes, mas em pontas opostas - embaixo no `A`, em cima no `D`. E é a emenda do loop: como eu sempre começo no `D` com a mão fresca, é a passagem que menos treino mesmo repetindo a volta inteira.
